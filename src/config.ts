...
export async function loadConfig(): Promise<Config> {
  ...
  const getEnv = (name: string): string | undefined => {
    try {
      return (
        (import.meta.env?.[`VITE_${name}`] as string) ||
  ...
  try {
    ...
    if (!backendCanisterId && config.backend_canister_id === "undefined") {
      console.error("CANISTER_ID_BACKEND is not set; IC features may be disabled.");
    }
    ...
    const fullConfig = {
      ...
      backend_canister_id: ((config.backend_canister_id === "undefined"
        ? backendCanisterId
        : config.backend_canister_id) || "unknown-canister-id") as string,
    ...
    if (!backendCanisterId) {
      console.error("CANISTER_ID_BACKEND is not set; using fallback config.");
    }
    const fallbackConfig = {
      backend_host: undefined,
      backend_canister_id: backendCanisterId || "unknown-canister-id",
